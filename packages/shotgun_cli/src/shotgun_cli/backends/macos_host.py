"""macOS-host backend: `flutter test -d macos` with surface-size overrides.

This is the Phase 1 backend, extracted as-is into the new backends
package. Behavior is unchanged — see `docs/ARCHITECTURE.md` for why
running the app inside a headless macOS Flutter test driver is the
fastest path to per-device-size captures without booting a simulator.
...
class MacosHostBackend:
    """Phase 1 backend. Runs the user's app inside `flutter test -d macos`
    with `setSurfaceSize` overrides per device — no simulator required."""
